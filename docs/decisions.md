# Dexora - decisions and history

This is the full record behind the rules in `CLAUDE.md`: what was reported, what was
measured, what was tried and reverted, and why each rule exists. It was CLAUDE.md itself
until the rules were split out into a short rulebook; it is kept verbatim so no reason is
lost. Where this file and CLAUDE.md disagree, CLAUDE.md and the code are current.

Search it by the rule's name, the constant, or the symptom.

---

A personal, non-commercial browser game: catch, collect and evolve the whole
National Dex from Kanto to Paldea plus its forms (count: `SPECIES.length`)
across eleven hand-made areas. Inspired by DelugeRPG's
...
`isForm(id)` (id >= 10000) is the single predicate, and three systems ask it:
...
`rareFor` adds each legendary to the maps that share its PRIMARY type
(`legendTier`), at `LEGEND_EACH` per head, capped at `LEGEND_CEIL`. Hand-writing
one into a table as well gives it two sets of odds in the same place.
...
**A variant is its own row in the Box.** Rows key on species AND variant.
...
every tier out of the spare list *entirely* rather than sorting them to the front —
sorting only ever protects the first `keep` of them. Bulk actions have no undo,
and at these odds there is no farming another.
...
"which ones are precious" is exactly how a third tier ships protected from one
bulk action and not another. Adding a fourth means adding it to `keeper()` and to
that list, and nowhere else.
**A test for this needs TWO of a tier** — with one, the reserve keeps it anyway
and the test passes with the protection deleted.
...
contiguous 1..N dex, and this one jumps from 1025 to 10001+ for its forms, so
`SPECIES[id - 1]` for a form is thousands of entries off the end. Both lookups
are Maps, built once, because the Dex grid asks per cell per render.
...
**There is an Origin honey now** (`honey-origin`). It was left out while
Origin sat behind a generation-completion gate, and it shipped once that gate
was deleted - every tier has its jar.
...
**An unheld Showdown cell draws the ordinary sprite.** The silhouette is a flat
fill of the outline either way, so there is no reason to load the strip for a
tier nobody owns.
